...
class PostController extends Controller
{
    ...
    public function store (CreateNewPostRequest $request)
    {
        ...
        if ($request->has('thumbnail')) {
            //có file upload
            //upload file vào trong storage
            $fileName = time().$request->file('thumbnail')->getClientOriginalName();
            try {
                //Upload file vào trong storage
                Storage::disk('public')->putFileAs('posts/',
                    $request->file('thumbnail'),
                    $fileName);

                //lưu đường dẫn vào trong db
                $instancePost = Post::find($post->id);
                $instancePost->thumbnail = $fileName;
                $instancePost->save();
            } catch (Exception $e) {
                dd($e);
            }
